...
def main():
    ...
    bm25_position = None
    passage_position = None

    for request, best_url in marks:
        ...
        if len(request_terms) == 0:
            bm25_position = 0
            passage_position = 0
            print '%s\t%d\t%d' % (request.encode('utf-8'), bm25_position, passage_position)
            continue
        ...
        sorted_bm25_doc_id = sorted([(doc_id, documents_rank[doc_id][BM25])  for doc_id in documents_rank.keys()],\
                                        key=lambda _tuple: _tuple[1], reverse=True)
        sorted_bm25_urls = [docID_to[doc_id][DOC_URL] for doc_id, rank in sorted_bm25_doc_id]

        try:
            bm25_position = sorted_bm25_urls.index(best_url) + 1
        except Exception, e:
            bm25_position = 0
            passage_position = 0
            print '%s\t%d\t%d' % (request.encode('utf-8'), bm25_position, passage_position)
        ...
        sorted_passage_urls = [docID_to[doc_id][DOC_URL] for doc_id, rank in final_ranking]

        try:
            passage_position = sorted_passage_urls.index(best_url) + 1
        except Exception, e:
            passage_position = 0
